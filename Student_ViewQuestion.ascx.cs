...
public partial class Student_ViewQuestion : System.Web.UI.UserControl
{
    ...
    protected void DataList1_ItemCommand(object source, DataListCommandEventArgs e)
    {
        if (e.CommandName == "Download")
        {
            ...
            cmd4.CommandText = "select * from tblQuestion where QuestionID=@id";
            cmd4.Parameters.AddWithValue("@id", qid);
            SqlDataReader dr4 = db4.executeread(cmd4);
            dr4.Read();
            string name = dr4.GetString(6);
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment;Filename=" + Path.GetFileName(name));
            Response.ContentType = "image/jpeg";
            Response.Flush();
            Response.TransmitFile(Server.MapPath("~/questionpaper/" + Path.GetFileName(name)));
            Response.End();
        }
    }
